...
- **The URL importer compares addresses numerically** (IPv4-mapped IPv6 unwrapped, `100.64.0.0/10` blocked, IPv6 allow-listed to global unicast). The name is still resolved twice.
...
- **Budget holds expire.** A reservation lives five minutes and is swept by the next reservation. Settlement never charges more than was reserved, and `import_url` treats `estimatedCost` as a size budget in KB.

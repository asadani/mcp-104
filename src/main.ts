...
import { actor,approve } from './policy.js';
...
app.get('/api/ops',async(req,res,next)=>{try{const a=await actor(db,await identity.subject(req.headers.authorization));const usage=(await db.query('SELECT spent,reserved,allowance FROM usage WHERE org=$1',[a.org])).rows[0];const auditRows=(await db.query('SELECT operation,outcome,trace_id,created_at FROM audit WHERE org=$1 ORDER BY created_at DESC LIMIT 12',[a.org])).rows;res.json({usage,audit:auditRows});}catch(e){next(e);}});
...
for(const signal of ['SIGINT','SIGTERM']) process.on(signal,()=>server.close(async()=>{await stopWorker?.();taskServer.close();knowledgeServer.close();await db.close();process.exit(0);}));
